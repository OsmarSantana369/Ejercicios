...
a)  **Obtener la matriz de dispersión de las 11 variables**
    ...
    ```{r, echo=FALSE}
    datos <- read.csv("C:/Users/GabDomDal/Desktop/Ejercicios/Analitica de datos/Trabajo 20/Cocodrilos.csv")
...
b)  **De la matriz de correlación, entre qué par de variables se da la mayor y menor relación lineal?**
    ...
    La mayor relación lineal se da entre la longitud del cráneo (CL) y la longitud dorsal del cráneo (DCL), con una correlación de 0.9990275 mientras que la menor se da entre la longitud del hocico (SL) y la ancho máximo orbital (OW) con una correlación de 0.7132771.
...
e)  **Realizar un ajuste del modelo quitando alguna(s) variables. Colocar la nueva ecuación de regresión con el modelo que creas que se ajusto mejor. ¿Hay un cambio significativo en el modelo?**
    ...
    -   **Modelo 2:** Solo se considerará la variable WCR, ya que la variable WN es la que tiene menor correlación con CW.
    ...
    De los modelos anteriores, se concluye que el que mejor se ajusta es el 1, ya que tiene mejor $R^2$ ajustado y menor error estándar. La ecuación del modelo es: $$ Y = -24.7710 + 1.5717*WN + 1.6228*WCR $$. Aunque el intercepto no es significativo, se decide mantenerlo en el modelo ya que algunos valores de la tabla cocodrilos tienen errores más reducidos.
