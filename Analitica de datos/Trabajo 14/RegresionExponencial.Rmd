...
```{r}
estimulo <- read.csv("C:/Users/GabDomDal/Desktop/Ejercicios/Analitica de datos/Trabajo 14/Estimulo.csv")
print(estimulo)
```
...
$$ \ln(Estimulo) = 1.6834 - 0.3329*Tiempo $$
...
```{r}
Cloro <- read.csv("C:/Users/GabDomDal/Desktop/Ejercicios/Analitica de datos/Trabajo 14/cloro.csv")
print(Cloro)
...
#Obtenemoslos valores obtenidos para b y c
library(tidyverse)
(fitData <- tibble(Semanas=1:50,fit = expo(Semanas,b_ajustado,c_ajustado)))

(nuevo <- tibble(Semanas=80,fit = expo(Semanas,b_ajustado,c_ajustado)))

#Graficamos el resultado de la regresi?n. 
ggplot()+
  geom_point(Cloro,mapping=aes(x=Semanas,y=Cloro)) +
  geom_line(fitData,mapping=aes(x=Semanas,y=fit),color="blue")
```
